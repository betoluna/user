...
public class Circular_Hough implements PlugInFilter {
    ...
    public void run(ImageProcessor ip) {
        ...
        if (readParameters()) {
            ...
            int thetaIncrement = Math.round(45 * minRad);
            ...
            // draw edges in new image with intensity value 127 *********************************
            int edgePos = 0;
            for (int y = 0; y < height; y++) {
                for (int x = 0; x < width; x++) {
                    if (imageValues[x + width * y] != 0) {
                        edges[edgePos] = 127;//max positive value in two's complement for a byte
                    } else {
                        edges[edgePos] = 0;
                    }
	...
	*/
	public void plotCircle(int CX, int CY, int R, ImageProcessor proc) {
		...
		while (X >= Y) {
			proc.set(CX + X, CY + Y, 255);// use set() instead of putPixel()
